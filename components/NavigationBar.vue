...
<template>
    <ClientOnly>
        <header
            ...
            <nav class="mx-auto w-full max-w-screen-2xl px-4 sm:flex sm:items-center sm:justify-between"
                aria-label="Global">
                <div class="flex items-center justify-between">
                    <NuxtLink class="flex-none font-title text-xl font-bold text-blue-600" to="/">
                        LearnLab
                    </NuxtLink>
                    <div class="sm:hidden">
                        <button type="button"
                            ...
                            <svg class="hidden size-4 flex-shrink-0 hs-collapse-open:block"
                ...
                <div id="navbar-collapse-with-animation"
                    ...
                    <div class="mt-5 flex flex-col gap-5 sm:mt-0 sm:flex-row sm:items-center sm:justify-end sm:ps-5">
                        <NuxtLink to="/" class="transition-color font-medium duration-200 ease-in-out" :class="route.path === '/'
                            ? 'text-blue-600 hover:text-blue-400'
                            : 'hover:text-gray-400'
                            " aria-current="page">
                            หน้าหลัก
                        </NuxtLink>
                        <NuxtLink v-if="userState?.u_role !== 'INSTRUCTOR'"
                            class="transition-color font-medium duration-200 ease-in-out" :class="route.path === '/courses'
                                ? 'text-blue-600 hover:text-blue-400'
                                : 'hover:text-gray-400'
                                " to="/courses" aria-label="Read more our courses offering">
                            หลักสูตร
                            <span v-if="!userState"
                        ...
                        <NuxtLink v-if="userState" class="transition-color font-medium duration-200 ease-in-out" :class="route.path === '/mycourse'
                            ? 'text-blue-600 hover:text-blue-400'
                            : 'hover:text-gray-400'
                            " to="/mycourse" aria-label="Read more our courses offering">
                            คอร์สเรียนของฉัน
                        </NuxtLink>
                        <TransitionGroup name="fade">
                            <button v-show="!userState" key="stateNone" type="button"
                                ...
                                <div class="w-22 flex items-center justify-center gap-2 overflow-hidden">
                                    <span class="material-icons-outlined w-[18px] h-[18px] overflow-hidden select-none"
                            ...
                            <div v-if="userState" key="stateLoggedIn"
                                ...
                                <div v-if="!avatarState?.u_avatar_blob_url"
                                ...
                                <div v-else class="h-8 w-8 rounded-md">
                                    <img class="aspect-square h-8 w-8 rounded-md object-cover"
                            ...
                            <Transition key="stateLoggedInName" name="fade">
                                <div v-show="userMenu"
                                    ...
                                    <div class="flex flex-col rounded-b-lg">
                                        <NuxtLink to="/settings" :class="route.path === '/settings'
                                        ...
                                        <div class="flex cursor-pointer items-center gap-x-2 rounded-b-lg bg-white p-4 text-red-600 hover:bg-gray-200"
                                            @click="signOut">
                                            <span class="material-icons-outlined size-6 overflow-hidden select-none">
                                                logout
                                            </span>
