...
async function fetchMember(id: number) {
    ...
    await $fetchWithHeader('/api/courses/enroll/', {
        ...
        .then((res: any) => {
            if (res?.length === 0) {
                toast.error('คอร์สนี้ไม่มีผู้สอน')
                crs_member.value = [{
                    "u_id": 0,
                    "c_id": id,
                    "u_role": "INSTRUCTOR",
                    "join_at": "1970-01-01 00:00:00",
                    "u_firstname": "ไม่มี",
                    "u_lastname": "ผู้สอน",
                    "u_email": "",
                    "u_avatar": false
    ...
    <div class="mx-auto mt-20 max-w-screen-2xl">
        <div class="flex flex-col gap-4 px-4 pb-16">
            ...
            <div class="relative flex">
                <img ref="bannerImage" class="h-full max-h-96 min-h-56 w-full rounded-xl object-cover object-[0%_50%]"
                    loading="lazy" :src="crs_info?.c_banner
                            ? `${runtimeConfig.public.apiBaseUrl}/api/courses/banner/?c_id=${crs_info?.c_id}`
                            : '/images/CourseBannerDefault.svg'
                        " alt="Image Description" @error="handleBrokenImage">
                <div
                ...
                <div v-if="getUserRoleInCurrentCourse(courseId) === 'INSTRUCTOR'" class="absolute left-0">
                    <div
                ...
                <div v-if="!crs_pending" class="absolute bottom-5 left-5 flex flex-col gap-2">
                    <span class="line-clamp-1 text-4xl font-bold text-white drop-shadow-[0_0_12px_rgba(0,0,0,1)]">
                        {{ crs_info?.c_name }}
                    </span>
                    <span
                ...
                <div v-else class="absolute bottom-5 left-5 flex w-full animate-pulse flex-col gap-2">
                    <span class="text-4xl font-bold text-white">
                        <h3 class="h-8 w-[200px] rounded-full bg-white" />
                    </span>
                    <span class="bottom-0 h-3 max-h-10 w-full rounded-full bg-white text-sm font-light text-white"
                        style="width: 90%" />
                </div>
            </div>
            <div class="flex md:flex-row flex-col gap-4">
                <nav class="border-1 flex h-fit w-full flex-col rounded-lg border shadow-sm md:w-64 flex-shrink-0">
                    ...
                    <div class="flex flex-col flex-wrap">
                        ...
                        <div v-for="inst in crs_member" :key="inst.u_id" class="flex flex-row items-center gap-2 p-2">
                            <div v-if="inst.u_avatar" class="h-8 w-8 rounded-md">
                                <img class="bottom-1 aspect-square rounded-md border object-cover"
                                    :src="`${runtimeConfig.public.apiBaseUrl}/api/avatar/?u_id=${inst.u_id}`">
                            </div>
                            <div v-if="!inst?.u_avatar"
