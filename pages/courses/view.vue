...
<template>
    ...
    <LazyCourseEditPostModal ref="editPostModal" :p_id="editPostId || 0" :p_title="editPostTitle || ''"
        :p_content="editPostContent || ''" :c_id="route.query.id" @refresh-post="fetchPost(route.query.id)" />
    <div class="flex flex-col gap-4 w-full">
        <div v-show="!crs_pending" class="flex flex-row justify-between items-center gap-4">
            <div></div>
            <div>
                <button type="button" @click="unenroll.c_openModal()"
                ...
                <button v-if="userRole?.[route.query.id] !== 'STUDENT'" type="button" @click="postModal.c_openModal"
        ...
        <div v-if="(crs_post?.data.length || 0) > 0" v-for="post in crs_post?.data"
            class="flex flex-col border border-1 rounded-md gap-2 w-full p-4">
            <div class="flex flex-wrap items-center justify-between gap-4 w-full">
                <div class="flex flex-row gap-4 items-center">
                    <div v-if="post.u_avatar" class="rounded-md w-12 h-12"><img
                            class="rounded-md aspect-square object-cover border bottom-1"
                            :src="`/api/avatar/?u_id=${post.u_id}`" /></div>
                    <div class="rounded-md w-12 h-12 bg-slate-200 flex flex-col justify-center items-center text-2xl select-none"
                        v-if="!post?.u_avatar">
                    ...
                    <div class="flex flex-col">
                        <span class="flex gap-2">
                            ...
                            <span
                                class="inline-flex items-center gap-x-1.5 py-0.5 px-1.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                {{ post.u_role === 'INSTRUCTOR' ? 'ผู้สอน' : post.u_role }}
                            </span>
                        </span>
                        <span class="text-sm text-slate-400">{{ new Date(post.p_updated_at).toLocaleString() }} {{
                            post.p_updated_at === post.p_created_at ? '' : '(ถูกแก้ไข)' }}</span>
                    </div>
                    <div v-if="post.p_type === 'ASSIGNMENT'" class="flex flex-col">
                        <span
                            class="inline-flex items-center gap-x-2 py-1 px-2 rounded-md text-sm font-medium bg-emerald-100 text-emerald-800">งานมอบหมาย</span>
                        <div class="flex flex-row flex-wrap"></div>
                    </div>
                    <div v-else-if="post.p_type === 'QUIZ'" class="flex flex-col">
                        <span
                            class="inline-flex items-center gap-x-2 py-1 px-2 rounded-md text-sm font-medium bg-orange-100 text-orange-800">แบบทดสอบ</span>
                        <div class="flex flex-row flex-wrap"></div>
                    </div>
                </div>
            ...
            <div class="flex flex-col" v-if="post.p_item_list.files.length">
                ...
                <div class="flex flex-row flex-wrap gap-2">
                    <div v-for="file in post?.p_item_list?.files"
                        class="flex flex-row border border-1 rounded-md md:w-72 w-full p-2 mt-1">
                        <div class="flex flex-row justify-between items-center gap-2 md:w-72 w-full">
                            <div class="flex flex-row items-center gap-2 overflow-hidden">
                                <div
                                    class="bg-slate-100 w-8 h-8 rounded-md flex flex-shrink-0 justify-center items-center select-none">
                                ...
                                <div class="flex flex-col md:w-48 w-10/12">
                                    <span class="text-xs whitespace-nowrap text-ellipsis overflow-hidden font-bold">{{
                                        file.f_name }}</span>
                                </div>
                            </div>
                            <div class="flex flex-row items-center gap-2 w-fit">
                                <span class="material-icons-outlined select-none cursor-pointer text-gray-500"
                                    @click="downloadFile(file.f_id)">download</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col" v-if="post.p_item_list.quizzes.length">
                ...
                <hr class="mb-2" />
                <div class="flex flex-row flex-wrap gap-2">
                    <div v-for="quiz in post?.p_item_list?.quizzes"
                        class="flex flex-row border border-1 rounded-md md:w-72 w-full p-2 mt-1">
                        <div class="flex flex-row justify-between items-center gap-2 md:w-72 w-full">
                            <div class="flex flex-row items-center gap-2 overflow-hidden">
                                <div
                                ...
                                <div class="flex flex-col md:w-48 w-10/12">
                                    <span class="text-xs whitespace-nowrap text-ellipsis overflow-hidden font-bold">{{
                                        quiz.q_name }}</span>
                                    <span class="text-xs whitespace-nowrap text-slate-400">{{ quiz.q_due_date ?
                                        `กำหนดส่ง ${new Date(quiz.q_due_date).toLocaleString()}` : 'ไม่มีกำหนดส่ง'
                                        }}</span>
                                    <span class="text-xs whitespace-nowrap text-slate-400" v-if="quiz.q_time_limit">{{
                                        quiz.q_time_limit ? `จำกัดเวลาทำ ${quiz.q_time_limit} นาที` : '' }}</span>
                                </div>
                            </div>
                            <div class="flex flex-row items-center gap-2 w-fit">
                                <span class="material-icons-outlined select-none cursor-pointer text-gray-500"
                                    @click="openQuiz(quiz.q_id)">open_in_new</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col" v-if="post.p_item_list.assignments.length">
                ...
                <hr class="mb-2" />
                <div class="flex flex-row flex-wrap gap-2">
                    <div v-for="assign in post?.p_item_list?.assignments"
                        class="flex flex-row border border-1 rounded-md md:w-72 w-full p-2 mt-1">
                        <div class="flex flex-row justify-between items-center gap-2 md:w-72 w-full">
                            <div class="flex flex-row items-center gap-2 overflow-hidden">
                                <div
                                ...
                                <div class="flex flex-col md:w-48 w-10/12">
                                    <span class="text-xs whitespace-nowrap text-ellipsis overflow-hidden font-bold">{{
                                        assign.a_name }}</span>
                                    <span class="text-xs whitespace-nowrap text-slate-400">{{ assign.a_due_date ?
                                        `กำหนดส่ง ${new Date(assign.a_due_date).toLocaleString()}` : 'ไม่มีกำหนดส่ง'
                                        }}</span>
                                    <span class="text-xs whitespace-nowrap text-slate-400" v-if="assign.a_score">{{
                                        assign.a_score }} คะแนน</span>
                                </div>
                            </div>
                            <div class="flex flex-row items-center gap-2 w-fit">
                                <span v-if="userRole?.[route.query.id] === 'STUDENT'"
                                ...
                                <span v-else class="material-icons-outlined select-none cursor-pointer text-gray-500"
        ...
        <div v-else-if="!crs_pending && (crs_post?.data.length || 0) === 0"
            class="flex md:flex-row flex-col items-center border border-1 rounded-md gap-2 w-full p-4">
