        ...
        <div v-if="!assignPending" class="flex w-full flex-col">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div class="w-full md:w-fit">
                    <button
v-if="!assignments?.s_datetime" type="button" :disabled="assignPending"
                    ...
                    <button
v-else-if="!assignments?.score" type="button" :disabled="assignPending"
                        class="transition-color inline-flex w-full items-center justify-center gap-x-2 rounded-lg border border-transparent bg-blue-600 px-6 py-2 text-sm font-semibold text-white duration-200 ease-in-out hover:bg-blue-700 disabled:pointer-events-none disabled:opacity-50 md:w-fit"
                    ...
                    <div v-else class="flex flex-col items-end justify-end">
                        ...
                        <span v-if="assignments?.s_feedback" class="text-slate-800">
            ...
            <div v-if="assignments?.a_files?.length" class="flex flex-row flex-wrap gap-2">
                <div
v-for="file in assignments?.a_files"
                    class="border-1 mt-1 flex w-full flex-row rounded-md border p-2 md:w-72">
                    <div class="flex w-full flex-row items-center justify-between gap-2 md:w-72">
...
class="material-icons-outlined cursor-pointer select-none text-gray-500"
        ...
        <div v-if="!assignPending" class="flex flex-col gap-4 overflow-y-auto">
            ...
            <div class="flex w-full flex-col gap-2 md:flex-row md:flex-nowrap">
                <input ref="inputFile" type="file" hidden @change="onFileChangedMat">
                <!-- End Floating Input -->
                <div>
                    <button
v-if="!assignments?.s_datetime" type="button" :disabled="assignPending"
                ...
                <div v-if="!assignments?.s_datetime" class="flex flex-col gap-x-4 gap-y-2 overflow-auto md:flex-row">
                    <TransitionGroup name="fade">
                        <div
v-for="(file, index) in submitFiles" :key="index + file.name"
                            class="flex items-center justify-between gap-2 rounded-md bg-blue-100 px-2 py-1.5 text-blue-600">
                            <div class="flex w-full flex-row flex-nowrap items-center gap-2 overflow-hidden">
                                <span class="material-icons-outlined select-none">
                                    insert_drive_file
                                </span>
                                <span class="w-full overflow-hidden text-ellipsis whitespace-nowrap text-xs md:w-24">
                                    {{ file.name }}
                                </span>
                            </div>
                            <span
class="material-icons-outlined cursor-pointer select-none text-red-500" @click="() => {
                                    if (index > -1) {
                ...
                <div class="">
                    <div v-if="assignments?.s_content?.files?.length" class="mt-4">
                        <span class="flex items-center gap-2 font-bold">
                            <span class="material-icons-outlined">attach_file</span>
                            ไฟล์ที่ส่งไป
                        </span>
                    ...
                    <div v-if="assignments?.s_content?.files?.length" class="flex flex-row flex-wrap gap-2">
                        <div
...
class="material-icons-outlined cursor-pointer select-none text-gray-500"
        ...
        <div v-else class="border-1 flex w-full flex-row gap-2 rounded-md border p-4">
            <div
