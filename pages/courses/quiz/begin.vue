    ...
    async function fetchQuiz(id: number, q_id: number) {
        await $fetch<QuizAPIResponse>('/api/courses/quiz/', {
            ...
            .then((res) => {
                ...
                quiz.value = quizResponse?.q_items?.[0]
            })
            .catch((err) => {
    ...
    <div class="flex flex-col gap-4 w-full">
        ...
        <div class="flex md:flex-row flex-col gap-2">
            <div class="flex flex-col flex-grow gap-4" >
                <div class="flex flex-col  border border-1 rounded-md flex-grow p-4 gap-4" v-for="(quiz, index) in quizResponse?.q_items">
                    <div class="flex flex-col gap-2">
                        <label class="text-2xl font-bold">{{index+1}}.) {{ quiz.title }}</label>
                    </div>
